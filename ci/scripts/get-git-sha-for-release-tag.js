#!/usr/bin/env node

const fs = require('fs')
const { Octokit } = require('@octokit/rest')
const { APPLICATION_IMAGE_TAG: imageTag, APP_NAME: appName, GITHUB_TOKEN: githubToken } = process.env
const octokit = new Octokit({ auth: githubToken })

async function run () {
  try {
    const gitTag = `alpha_release-${imageTag}`.replace('-release', '')
    const refResult = await octokit.git.getRef({ owner: 'govuk-pay', repo: `pay-${appName}`, ref: `tags/${gitTag}` })
    const tagSha = refResult.data.object.sha
    const tagResult = await octokit.git.getTag({ owner: 'govuk-pay', repo: `pay-${appName}`, tag_sha: `${tagSha}` })
    console.log(`git sha: ${tagResult.data.object.sha}`)
    fs.writeFileSync('git-sha/git-sha', tagResult.data.object.sha)
  } catch (err) {
    console.log(`An error occurred: ${err}`)
    process.exitCode = 1
  }
}

run()
